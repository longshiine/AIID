...
for idx,line in enumerate(rdr):
  ...
  if len(line[6].split()) > 3:
    if '로' in line[6] or '길' in line[6]:
      l = requests.get('http://api.vworld.kr/req/address?service=address&request=getcoord&version=2.0&crs=epsg:4326&address='+line[6]+'&refine=true&simple=false&format=json&type=ROAD&key='+apiKey)
    else:
      l = requests.get('http://api.vworld.kr/req/address?service=address&request=getcoord&version=2.0&crs=epsg:4326&address='+line[6]+'&refine=true&simple=false&format=json&type=PARCEL&key='+apiKey)
      
    result = dict(l.json())['response']
    if result['status'] == 'OK':
      JUSO = ""
      # 동/읍/면으로 필터링
      for idx, juso in enumerate(result['refined']['text'].split()):
        if '도' in juso or '시' in juso or '군' in juso or '구' in juso or '동' in juso or '읍' in juso or '면' in  juso:
          ...
          continue
      X = result['result']['point']['x']
      Y = result['result']['point']['y']
      row += [JUSO, X, Y] + line[10:]
      wdr.writerow(row)
      print(row)
  # else:
  #   row += [line[6],0,0]
